...
from src.utils.o3d_helper import convert_to_o3d_pointcloud, downsampling_pc, convert_to_numpy_array
...
def __patch_scene(scene_id: str,
    ...
    point_cloud_accumulator = PointCloudAccumulator(step=3,
    ...
    for instance in grouped_instances.keys():
        ...
        # todo: may be not use downsampling for semseg
        # pc_o3d = convert_to_o3d_pointcloud(points=accumulated_point_cloud.T)
        # pc_o3d_down = downsampling_pc(o3d_pc=pc_o3d, voxel_size=0.07)
        # accumulated_point_cloud = convert_to_numpy_array(o3d_pc=pc_o3d_down)
    ...
    for instance, frames in grouped_instances.items():
        for frame_id in frames:
            ...
            if can_skip_frame(dataset=dataset,
                              ...
                              frame_id=frame_id,
                              force_overwrite=force_overwrite):
                print(f"[Scene {scene_id}] Skipping frame {frame_id}...")
...
def parse_arguments():
    ...
    parser.add_argument('--dataset', type=str, choices=['nuscenes', 'once', 'waymo', 'pointcept'], default='pointcept',
                        help='Dataset.')
    ...
    parser.add_argument('--dataroot', type=str, default='./data/waymo/', help='Data root location.')
...
def main():
    ...
    if dataset_type == 'pointcept':
        dataset = PointceptDataset(dataset_root=args.dataroot)

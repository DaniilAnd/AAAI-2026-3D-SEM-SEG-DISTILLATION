...
class GridSampleDistill(object):
    def __init__(self, grid_size=0.05, hash_type="fnv", mode="train", keys=("coord", "strength", "segment"),
        ...
        self.original_transform = TRANSFORMS.build(dict(
            type="GridSample",
            grid_size=grid_size,
            hash_type=hash_type,
            mode=mode,
    ...
    def __call__(self, data_dict):
        ...
        # Process teacher
        teacher = self.apply_grid_sample(teacher)
        
        # Restore random state for student
        np.random.set_state(numpy_state)
        torch.random.set_rng_state(torch_state)
        
        # Process student
        student = self.apply_grid_sample(student)
        
        data_dict['teacher'] = teacher
        data_dict['student'] = student
        return data_dict

    def apply_grid_sample(self, data):
        # Create temporary dict for standard GridSample processing
        temp_dict = {k: data[k] for k in self.keys}
        temp_dict = self.original_transform(temp_dict)
        
        # Update data with processed results
        for k in temp_dict.keys():
            data[k] = temp_dict[k]
        if self.return_grid_coord and "grid_coord" in temp_dict:
            data["grid_coord"] = temp_dict["grid_coord"]
        return data


@TRANSFORMS.register_module()
class ToTensorDistill(object):
    def __init__(self):
        self.original_transform = TRANSFORMS.build(dict(type="ToTensor"))

    def __call__(self, data_dict):
        data_dict['teacher'] = self.apply_to_tensor(data_dict['teacher'])
        data_dict['student'] = self.apply_to_tensor(data_dict['student'])
        return data_dict

    def apply_to_tensor(self, data):
        return self.original_transform(data)


@TRANSFORMS.register_module()
class CollectDistill(object):
    def __init__(self, keys=("coord", "grid_coord", "segment"), feat_keys=("coord", "strength")):
        self.keys = keys
        self.feat_keys = feat_keys
        self.teacher_prefix = "teacher_"
        self.student_prefix = "student_"

    def __call__(self, data_dict):
        # Process teacher
        teacher_data = self.apply_collect(data_dict['teacher'], prefix=self.teacher_prefix)
        
        # Process student
        student_data = self.apply_collect(data_dict['student'], prefix=self.student_prefix)
        
        # Merge results
        data_dict.update(teacher_data)
        data_dict.update(student_data)
        return data_dict

    def apply_collect(self, data, prefix):
        result = {}
        ...
        # Handle feature concatenation
        feats = []
        for feat_key in self.feat_keys:
            if feat_key in data:
                feats.append(data[feat_key])
        if feats:
            result[prefix + "feat"] = np.concatenate(feats, axis=1)
        
        return result
    
@TRANSFORMS.register_module()
class RandomRotateDistill(object):
    ...
    def __call__(self, data_dict):
        teacher_input = data_dict['teacher']
        student_input = data_dict['student']
        
        # Save original random state
        numpy_state = np.random.get_state()
        torch_state = torch.random.get_rng_state()
        
        # Apply same transform to teacher and student
        teacher_input, student_input = self.apply_transform(teacher_input, student_input)
        
        # Restore random state
        np.random.set_state(numpy_state)
        torch.random.set_rng_state(torch_state)
        
        data_dict['teacher'] = teacher_input
        data_dict['student'] = student_input
        return data_dict

    def apply_transform(self, teacher, student):
        ...
        # Apply to teacher
        if "coord" in teacher:
            center = self.get_center(teacher["coord"])
            teacher["coord"] = self.apply_rotation(teacher["coord"], rot_t, center)
            if "normal" in teacher:
                teacher["normal"] = np.dot(teacher["normal"], rot_t.T)

        # Apply to student
        if "coord" in student:
            center = self.get_center(student["coord"])
            student["coord"] = self.apply_rotation(student["coord"], rot_t, center)
            if "normal" in student:
                student["normal"] = np.dot(student["normal"], rot_t.T)

        return teacher, student

    def get_center(self, coord):
        if self.center is None:
            return [(coord[:,0].min()+coord[:,0].max())/2, 
                    (coord[:,1].min()+coord[:,1].max())/2,
                    (coord[:,2].min()+coord[:,2].max())/2]
        return self.center

    def apply_rotation(self, coord, rot_t, center):
        coord = coord - center
        coord = np.dot(coord, rot_t.T)
        return coord + center


@TRANSFORMS.register_module()
class RandomFlipDistill(object):
    ...
    def __call__(self, data_dict):
        ...
        # Apply to teacher
        if flip_x and "coord" in teacher_input:
            teacher_input["coord"][:, 0] = -teacher_input["coord"][:, 0]
            if "normal" in teacher_input:
                teacher_input["normal"][:, 0] = -teacher_input["normal"][:, 0]
        
        if flip_y and "coord" in teacher_input:
            teacher_input["coord"][:, 1] = -teacher_input["coord"][:, 1]
            if "normal" in teacher_input:
                teacher_input["normal"][:, 1] = -teacher_input["normal"][:, 1]
        
        # Restore random state and apply same flags to student
        np.random.set_state(numpy_state)
        
        if flip_x and "coord" in student_input:
            student_input["coord"][:, 0] = -student_input["coord"][:, 0]
            if "normal" in student_input:
                student_input["normal"][:, 0] = -student_input["normal"][:, 0]
        
        if flip_y and "coord" in student_input:
            student_input["coord"][:, 1] = -student_input["coord"][:, 1]
            if "normal" in student_input:
                student_input["normal"][:, 1] = -student_input["normal"][:, 1]
...
@TRANSFORMS.register_module()
class RandomJitterDistill(object):
    ...
    def __call__(self, data_dict):
        ...
        if "coord" in teacher:
            jitter = np.clip(
                self.sigma * np.random.randn(*teacher["coord"].shape),
                -self.clip, 
                self.clip
            )
            
            # Apply same jitter to both
            teacher["coord"] += jitter

        if "coord" in student:
            jitter = np.clip(
                self.sigma * np.random.randn(*student["coord"].shape),
                -self.clip, 
                self.clip
            )
            # Apply same jitter to both
            student["coord"] += jitter
...
@TRANSFORMS.register_module()
class RandomScaleDistill(object):
    ...
    def __call__(self, data_dict):
        ...
        # Generate scale parameters once
        numpy_state = np.random.get_state()
        scale = self.generate_scale(teacher["coord"].shape)
        
        # Apply to teacher
        teacher = self.apply_scale(teacher, scale)
        
        # Restore state and apply same scale to student
        np.random.set_state(numpy_state)
        student = self.apply_scale(student, scale)
        
        data_dict['teacher'] = teacher
        data_dict['student'] = student
        return data_dict

    def generate_scale(self, shape):
        if self.anisotropic:
    ...
    def apply_scale(self, data, scale):
        if "coord" in data:
            # Apply same scale to all points
            if self.anisotropic:
                data["coord"] *= scale
            else:
                data["coord"] *= scale.item()
        return data

@TRANSFORMS.register_module()
class PointClipDistill(object):
    def __init__(self, point_cloud_range=(-75.2, -75.2, -4, 75.2, 75.2, 2)):
        self.point_cloud_range = point_cloud_range

    def __call__(self, data_dict):
        data_dict['teacher'] = self.apply_clip(data_dict['teacher'])
        data_dict['student'] = self.apply_clip(data_dict['student'])
        return data_dict
